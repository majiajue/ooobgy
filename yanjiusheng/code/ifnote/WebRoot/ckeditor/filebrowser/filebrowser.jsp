...
<body  scroll=yes>
<form action="ckeditor/browser" id="imgbrowser" name="imgbrowser">
<%
String baseUrl = "\\UserFiles";
String realPath = application.getRealPath(baseUrl);
//System.out.println("浏览页得到的真实路径:" + realPath); 
String baseFileUrl = application.getServletContextName() + "/UserFiles/";
System.out.println("baseUrl:" + baseFileUrl);
int col = 0;//计数显示的列数
final int MAX_COL = 6;
int row = 0;
...
<input type="hidden" id="appUrl" name="appUrl" value="<%=application.getServletContextName() %>"/>
...
%>
  ...
  <tr>
    ...
    <div align="center">
    <%if(typeId == 1){ %>  
    <img src="/energySaving/ckeditor/zoomimg?url=<%=imgname%>&targetW=100&targetH=100" style="cursor:hand;" ondblclick="javascript:nowsubmit('<%=imgname%>');" onclick="javascript:transurl('<%=imgname%>');"/></div></a>
   <%}else if(typeId == 0){ %>
   <img src="/energySaving/ckeditor/images/fileicon.jpg" style="cursor:hand;" ondblclick="javascript:nowsubmit('<%=imgname%>');" onclick="javascript:transurl('<%=imgname%>');"/></div></a>
   <%}else if(typeId == 2){ %>
   <img src="/energySaving/ckeditor/images/flashicon.jpg" style="cursor:hand;" ondblclick="javascript:nowsubmit('<%=imgname%>');" onclick="javascript:transurl('<%=imgname%>');"/></div></a>
   <%} %>
</td>
  </tr>
  <tr>
...
<div align="center"><img src="/energySaving/ckeditor/images/chosefile.png" style="cursor:hand;" onclick="javascript:endsub();"/></div>
</form>
</body>
</html>
 ...
 * 
 */
public class GetPathNOTMatchered extends Configured implements Tool {
				...
				try {
					...
					if (nodeFeatureAttrs.size() == 1) {
						...
						if (cts == null || cts.size() == 0) {
							...
							StringBuilder uidUnMatchPath = new StringBuilder();
							uidUnMatchPath.append(uid).append("\t");
							uidUnMatchPath.append(rawTrace);
							output.collect(new Text(key.toString()), new Text(uidUnMatchPath.toString()));
						}
//						outputHeadAtomTraces(ats, nodeIdsAttrs, output, reporter);
//						outputCompositeTraces(rawTrace, cts, nodeIdsAttrs, output, reporter);
		 ...
		 */
		private void outputHeadAtomTraces(List<AtomTrace> ats, Map<String, JSONObject> nodeIdsAttrs, OutputCollector<Text, Text> output, Reporter reporter) throws JSONException, IOException {
			for(AtomTrace at:ats){
				if (invertedList.isHeadNode(at.startNode)){
					...
					outputAtomTrace(headAts, nodeIdsAttrs, output, reporter);
				}
			}
		}
		 ...
		 * @throws IOException 
		 */
		private void outputAtomTrace(List<AtomTrace> ats, Map<String, JSONObject> nodeAttrs, OutputCollector<Text, Text> output, Reporter reporter) throws JSONException, IOException {
			if (ats==null || ats.size() <=0 || nodeAttrs.size() <=0 ){
				reporter.incrCounter("PathCounters", "WrongAtomTraces", 1);
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			pizzaJob.setJobName("GetPathNOTMatchered");

			pizzaJob.setJarByClass(GetPathNOTMatchered.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new JobConf(), new GetPathNOTMatchered(), args);
		System.exit(res);
	}
}

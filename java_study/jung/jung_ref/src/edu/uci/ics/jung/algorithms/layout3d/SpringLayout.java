    ...
    public void step() {
    	...
    	try {
    		for(V v : getGraph().getVertices()) {
    			SpringVertexData svd = getSpringDataV(v);
    ...
    protected void relaxEdges() {
    	try {
    		for(E e : getGraph().getEdges()) {
    			...
    			v1D = getSpringDataV(v1);
    			v2D = getSpringDataV(v2);
    ...
    protected void moveNodes() {
        ...
        synchronized (getSize()) {
            try {
                for (V v : getGraph().getVertices()) {
                    if (isLocked(v)) continue;
                    SpringVertexData vd = getSpringDataV(v);
                    if(vd == null) continue;
...
//                    System.err.println(v+" xyd = "+xyd);
    ...
    public SpringVertexData getSpringDataV(V v) {
